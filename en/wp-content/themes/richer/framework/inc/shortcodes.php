...
function richer_clients($atts, $content = null) {
  $output = '<script type="text/javascript">
            jQuery(window).load(function() {
              jQuery("#flexslider-clients").flexslider({
                animation: "slide",
                smoothHeight : true,
                directionNav: true,
                controlNav: false,
...
function richer_video($atts) {
  ...
  if($type == "vimeo") $return = "<div class='video-embed'><iframe src='http://player.vimeo.com/video/$id?autoplay=$autoplay&amp;title=0&amp;byline=0&amp;portrait=0&amp;color=".$options_data['color_accent']."' width='$width' height='$height' class='iframe'></iframe></div>";
...
if($filter == 'yes' && $cat_slug == ''){
  ...
  wp_register_script('isotope-js', get_template_directory_uri() . '/framework/js/isotope.js', 'jquery', '1.5');
  wp_enqueue_script('isotope-js');
  $filter_out = "<script type='text/javascript'>
  jQuery(window).load(function(){
  var container = jQuery('#portfolio-wrap-".$folio_id."');
  // initialize isotope
  container.isotope({
  ...
  jQuery('#filters.filter-".$folio_id." a').click(function(){
      ...
      $out .= '<li><div class="portfolio-item no-margin">';     
        ...
        if ( has_post_thumbnail()) {
            ...
            if (get_post_meta( get_the_ID(), 'richer_source', true ) == 'vimeo') {  
                $out .= '<div id="portfolio-video"><iframe src="http://player.vimeo.com/video/'.get_post_meta( get_the_ID(), 'richer_embed', true ).'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" width="470" height="340" style="border-width:0px" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';  
            }  
            else if (get_post_meta( get_the_ID(), 'richer_source', true ) == 'youtube') {  
                $out .= '<div id="portfolio-video"><iframe width="470" height="340" src="http://www.youtube.com/embed/'.get_post_meta( get_the_ID(), 'richer_embed', true ).'?rel=0&amp;showinfo=0&amp;modestbranding=1&amp;hd=1&amp;autohide=1&amp;color=white" style="border-width:0px" allowfullscreen></iframe></div>';  
            }  
            else {  
                $out .= '<div id="portfolio-video">'.get_post_meta( get_the_ID(), 'richer_embed', true ).'</div>';
            }  
        } else {
  ...
  case 'grid-masonry':
    ...
    while ( have_posts() ) : the_post();
      ...
      if(!empty($images) && get_post_meta( $post->ID, 'richer_gridlayout', true ) != "true"){ 
        $out .= "<script>
          jQuery(window).load(function(){
            jQuery('.portfolio-slider-".$randomid."').flexslider({
              animation: 'fade',
              smoothHeight: true,
              controlNav: false,
              directionNav: true,
        ...
        if (get_post_meta( get_the_ID(), 'richer_source', true ) == 'vimeo') {  
            $out .= '<div id="portfolio-video"><iframe src="http://player.vimeo.com/video/'.get_post_meta( get_the_ID(), 'richer_embed', true ).'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" width="770" height="400" style="border-width:0px" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';  
        }  
        else if (get_post_meta( get_the_ID(), 'richer_source', true ) == 'youtube') {  
            $out .= '<div id="portfolio-video"><iframe width="770" height="400" src="http://www.youtube.com/embed/'.get_post_meta( get_the_ID(), 'richer_embed', true ).'?rel=0&amp;showinfo=0&amp;modestbranding=1&amp;hd=1&amp;autohide=1&amp;color=white" style="border-width:0px" allowfullscreen></iframe></div>';  
        }  
        else {  
            $out .= '<div id="portfolio-video">'.get_post_meta( get_the_ID(), 'richer_embed', true ).'</div>';
        }  
  ...
  if( have_posts() ) :
    switch($layout) {
      ...
      $out .= '<div id="blog_items_wrap">';
        ...
        $out .= '<div class="blog-item">';
          if($show_thumb != 'no' && has_post_thumbnail()){
            ...
            if (get_post_meta( get_the_ID(), 'richer_source', true ) == 'vimeo') {  
                $out .= '<div id="blog-video"><iframe src="http://player.vimeo.com/video/'.get_post_meta( get_the_ID(), 'richer_embed', true ).'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" '.$video_size.' style="border-width:0px" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';  
            }  
            else if (get_post_meta( get_the_ID(), 'richer_source', true ) == 'youtube') {  
                $out .= '<div id="blog-video"><iframe '.$video_size.' src="http://www.youtube.com/embed/'.get_post_meta( get_the_ID(), 'richer_embed', true ).'?rel=0&amp;showinfo=0&amp;modestbranding=1&amp;hd=1&amp;autohide=1&amp;color=white" style="border-width:0px" allowfullscreen></iframe></div>';  
            }  
            else {  
                $out .= '<div id="blog-video">'.get_post_meta( get_the_ID(), 'richer_embed', true ).'</div>';
            }  
        }
          if($show_author != 'no'){
          ...
          $out .= '<div class="date"><span>'.get_the_time('F j, Y').'</span></div>';
          $out .= '<div class="blog-item-excerpt">'.my_string_limit_words(get_the_excerpt(), $excerpt_count).' ';
          if($show_readmore != 'no') {
      ...
      case 'carousel':
        $out .= '<script type="text/javascript">
        jQuery(window).load(function() {
          jQuery("#flexslider-blog-'.$blog_id.'").flexslider({
            animation: "slide",
            slideshow: '.$slideshow.',
            smoothHeight : true,
            directionNav: true,
...
$post = get_post();
  ...
  foreach ( $attachments as $id => $attachment ) {
    if ( ! empty( $link ) && 'file' === $link )
      $image_output = '<a href="'.wp_get_attachment_image_src( $id, 'full', false, false ).'" rel="prettyPhoto['.$selector.']">'.wp_get_attachment_image( $id, $size, false ).'</a>';
