...
<?php get_template_part('framework/inc/portfolio/folio-filter')?>
	<div class="span12">
		<div id="portfolio-wrap">
			<?php
				global $wp_query;
				$portfolioitems = $options_data['text_portfolioitems_2']; // Get Items per Page Value
				...
				<div class="<?php if($terms) : foreach ($terms as $term) { echo 'term-'.$term->slug.' '; } endif; ?>portfolio-item span6 no-margin">
					...
					<?php } else if (get_post_meta( get_the_ID(), 'richer_embed', true ) != '') {  
					    if (get_post_meta( get_the_ID(), 'richer_source', true ) == 'vimeo') {  
					        echo '<div id="portfolio-video"><iframe src="http://player.vimeo.com/video/'.get_post_meta( get_the_ID(), 'richer_embed', true ).'?title=0&amp;byline=0&amp;portrait=0&amp;color=ffffff" width="583" height="303" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';  
					        ...
					        echo '<div id="portfolio-video"><iframe width="583" height="303" src="http://www.youtube.com/embed/'.get_post_meta( get_the_ID(), 'richer_embed', true ).'?rel=0&showinfo=0&modestbranding=1&hd=1&autohide=1&color=white" frameborder="0" allowfullscreen></iframe></div>';  
					    	echo '<div class="portfolio-title"><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a></div><div class="clear"></div>';
					    }  
					    else {  
