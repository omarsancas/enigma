...
if(is_numeric($d) && $d > 0) {
	...
	if($d < 3) return __("right now",'richer');
	//if less then minute
	if($d < $minute) return floor($d) . __(" seconds ago",'richer');
	//if less then 2 minutes
	if($d < $minute * 2) return __("about 1 minute ago",'richer');
	//if less then hour
	if($d < $hour) return floor($d / $minute) . __(" minutes ago",'richer');
	//if less then 2 hours
	if($d < $hour * 2) return __("about 1 hour ago",'richer');
	//if less then day
	if($d < $day) return floor($d / $hour) . __(" hours ago",'richer');
	//if more then day, but less then 2 days
	if($d > $day && $d < $day * 2) return __("yesterday",'richer');
	//if less then year
	if($d < $day * 365) return floor($d / $day) . __(" days ago",'richer');
	//else return more than a year
	return __("over a year ago",'richer');
	}
}
	...
	class widget_twitter extends WP_Widget {
		...
		public function __construct() {
			parent::__construct(
				...
				'richer-Twitter', // Name
				array( 'description' => __( 'Display recent tweets', 'richer'), ) // Args
			);
		}
		//widget output
		public function widget($args, $instance) {
			...
			$tp_twitter_plugin_last_tweetstoshow = get_option('tp_twitter_plugin_last_tweetstoshow');
			$diff_twets = $instance['tweetstoshow'] != $tp_twitter_plugin_last_tweetstoshow ? true : false;
			$diff = time() - $tp_twitter_plugin_last_cache_time;
			$crt = $instance['cachetime'] * 3600;
			$tweets_array = '';
			//	yes, it needs update			
			if($diff >= $crt || empty($tp_twitter_plugin_last_cache_time) || $diff_twets){
				...
				update_option('tp_twitter_plugin_last_tweetstoshow',serialize($instance['tweetstoshow']));
			...
			if(!empty($tp_twitter_plugin_tweets) && is_array($tp_twitter_plugin_tweets)){
				print '<div class="widget_twitter">';
				$fctr = '1';
				foreach($tp_twitter_plugin_tweets as $tweet){								
					print '<div class="tweet"><a target="_blank" href="http://twitter.com/'.$instance['username'].'/">@'.$instance['username'].'</a><span> '.convert_links($tweet['text']).'</span><br /><span class="date">'.relative_time($tweet['created_at']).'</span></div>';
		...
		public function form($instance) {
			$defaults = array( 'title' => 'Recent tweets', 'consumerkey' => '', 'consumersecret' => '', 'accesstoken' => '', 'accesstokensecret' => '', 'cachetime' => '5', 'username' => '', 'tweetstoshow' => '2' );
			$instance = wp_parse_args( (array) $instance, $defaults );
					
			echo '
			<p><label>'.__('Title:','richer').'</label>
			...
			<p><label>'.__('Consumer Key:','richer').'</label>
			...
			<p><label>'.__('Consumer Secret:','richer').'</label>
			...
			<p><label>'.__('Access Token:','richer').'</label>
			...
			<p><label>'.__('Access Token Secret:','richer').'</label>		
			...
			<p><label>'.__('Cache Tweets in every:','richer').'</label>
			...
			<p><label>'.__('Twitter Username:','richer').'</label>
			...
			<p><label>'.__('Tweets to display:','richer').'</label>
