...
class sidebar_generator {
			...
			<script>
				function add_sidebar( sidebar_name )
				{
					...
					var mysack = new sack("<?php echo site_url(); ?>/wp-admin/admin-ajax.php" );    
				  	...
				  	mysack.onError = function() { alert("<?php _e('Ajax error. Cannot add sidebar','richer-framework'); ?> ")};
				  	mysack.runAJAX();
					return true;
				}
	...
	public static function add_sidebar(){
		...
		if(isset($sidebars[$id])){
			die("alert('".__('Sidebar already exists, please use a different name.','richer-framework')."')");
		}
		...
		$js = "
			...
			//var textNode = document.createTextNode('[<a href=\'javascript:void(0);\' onclick=\'return remove_sidebar_link($name);\'>".__('Remove','richer-framework')."</a>]');
			//cellLeft.appendChild(textNode)
	...
	public static function remove_sidebar(){
		...
		$id = sidebar_generator::name_to_class($name);
		if(!isset($sidebars[$id])){
			die("alert('".__('Sidebar does not exist.','richer-framework')."')");
		}
	...
	public static function admin_page(){
		...
		<script>
			function remove_sidebar_link(name,num){
				answer = confirm("<?php _e('Are you sure you want to remove','richer-framework'); ?> " + name + "<?php _e('?\nThis will remove any widgets you have assigned to this sidebar.','richer-framework'); ?>");
				if(answer){
					//alert('AJAX REMOVE');
			...
			function add_sidebar_link(){
				var sidebar_name = prompt("<?php _e('Sidebar Name:','richer-framework'); ?>","");
				//alert(sidebar_name);
				add_sidebar(sidebar_name);
			}
		</script>
		<div class="wrap">
			<h2><?php _e('Sidebar Generator','richer-framework'); ?></h2>
			
			<p><?php _e('You can generate as many sidebars as you need. Then you can place them on any page you wish.','richer-framework'); ?></p>
			<br />
			<table class="widefat page" id="sbg_table" style="width:600px;">
				<tr>
					<th><?php _e('Name','richer-framework'); ?></th>
					<th><?php _e('CSS class','richer-framework'); ?></th>
					<th><?php _e('Remove','richer-framework'); ?></th>
				</tr>
				<?php
				$sidebars = sidebar_generator::get_sidebars();
				...
				<tr class="<?php echo $alt?>">
					...
					<td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo $sidebar; ?>',<?php echo $cnt+1; ?>);" title="<?php _e('Remove this sidebar','richer-framework'); ?>"><?php _e('remove','richer-framework'); ?></a></td>
				</tr>
				<?php
						$cnt++;
					}
					...
					?>
					<tr>
						<td colspan="3"><?php _e('No Sidebars defined','richer-framework'); ?></td>
					</tr>
					<?php
				}
				?>
			...
			<div class="add_sidebar">
				<a href="javascript:void(0);" class="button" onclick="return add_sidebar_link()" title="<?php _e('Add a sidebar','richer-framework'); ?>"><?php _e('Create Sidebar','richer-framework'); ?></a>
			</div>
		</div>
		<?php
	}
	...
	*/
	...
	<div id='sbg-sortables' class='meta-box-sortables'>
		<div id="sbg_box" class="postbox " >
			<div class="handlediv" title="<?php _e('Click to toggle','richer-framework'); ?>"><br /></div><h3 class='hndle'><span><?php _e('Sidebars','richer-framework'); ?></span></h3>
			<div class="inside">
				<div class="sbg_container">
					<input name="sbg_edit" type="hidden" value="sbg_edit" />
					
					<p>
						<?php _e("Select the sidebar you wish to display on this page. If you don't want to display a Sidebar, select the 'Full-Width' Template.<br />
						<strong>Note:</strong> You must first create the sidebar under Appearance > Sidebars.",'richer-framework'); ?>
					</p>
					<ul>
					<?php 
					...
					//var_dump($wp_registered_sidebars);		
						for($i=0;$i<1;$i++){ ?>
							...
							<select name="sidebar_generator[<?php echo $i?>]" style="display: none;">
								<option value="0"<?php if($selected_sidebar[$i] == ''){ echo " selected";} ?>><?php _e('WP Default Sidebar','richer-framework'); ?></option>
							<?php
