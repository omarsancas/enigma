...
jQuery(document).ready(function($){
	...
	//my radio-buttons settings
	function show_hide_boxes(val) {
        jQuery('.f_header_layout[data-show*='+val+']').css({'display':'block'});
	}
	jQuery('.f_header_layout').css({'display':'none'});
	show_hide_boxes(jQuery('input:radio[name="header_layout"]:checked').val());
	jQuery('.section-images span img').click(function(){
		jQuery('.f_header_layout').css({'display':'none'});
	    show_hide_boxes(jQuery(this).parent().find('input:radio[name="header_layout"]').val());
	});
	...
	/**	Tipsy @since v1.3 */
	if (jQuery().tipsy) {
		$('.tooltip, .typography-size, .typography-height, .typography-face, .typography-transform, .typography-style, .of-typography-color').tipsy({
			fade: true,
			gravity: 's',
			opacity: 0.7,
		});
